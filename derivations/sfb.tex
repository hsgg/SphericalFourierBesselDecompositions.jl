...
\,.
\ea
Thus, we need only minimal changes to the mixing matrix for auto-correlation:
\ba
...
\begin{widetext}
We can further express $W_{LM}(r)$ as a full SFB transform. We get
\ba
\mathcal{M}_{\ell nn'}^{XY,LNN'}
&=
\frac{2L+1}{4\pi}
...
\sum_{M_1}
\vs&\quad\times
\int\dd r\,r^2 \,g_{n\ell}(r) \,g_{NL}(r) \sum_{N_1} g_{N_1L_1}(r)\, W^X_{N_1 L_1 M_1}
\vs&\quad\times
\int\dd r'\,r'^2 \,g_{n'\ell}(r') \,g_{N'L}(r') \sum_{N_2} g_{N_2 L_1}(r')\,W^{Y,*}_{N_2 L_1 M_1}
\\
&=
\frac{2L+1}{4\pi}
\sum_{L_1}
...
\end{pmatrix}^2
\sum_{N_1}
\int\dd r\,r^2 \,g_{n\ell}(r) \,g_{NL}(r)\, g_{N_1L_1}(r)
\vs&\quad\times
\sum_{N_2}
\int\dd r'\,r'^2 \,g_{n'\ell}(r') \,g_{N'L}(r')\, g_{N_2 L_1}(r')
\,\frac{1}{2L_1 + 1}
\sum_{M_1} W^X_{N_1 L_1 M_1} \, W^{Y,*}_{N_2 L_1 M_1}
\\
&=
\frac{2L+1}{4\pi}
\sum_{L_1}
...
\end{pmatrix}^2
\sum_{N_1}
\left[\int\dd r\,r^2 \,g_{n\ell}(r) \,g_{NL}(r)\, g_{N_1L_1}(r)\right]
\vs&\quad\times
\sum_{N_2}
\left[\int\dd r'\,r'^2 \,g_{n'\ell}(r') \,g_{N'L}(r')\, g_{N_2 L_1}(r')\right]
\,C^{W^XW^Y}_{L_1 N_1 N_2}
\,.
\ea
The integrals in brackets will be very expensive to calculate.
\end{widetext}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Semi-separable window function}
It would be incredibly convenient to approximate the full window function as
a sum of separable terms, i.e.,
\ba
W^X(r, \rhat) &= \sum_i \phi^X_i(r)\,M^X_i(\rhat)\,.
\label{eq:semi_separable_window}
\ea
This decomposition is not unique. We should choose carefully. An obvious way
would be to split the sky into sections, each section being separable.

Defining the window for each section,
\ba
\label{eq:win_r_lm}
W^{Xi}_{LM}(r)
&=
\phi^{Xi}(r)
\int\dd^2\rhat\,Y^*_{LM}(\rhat)
\,M^{Xi}(\rhat)\,,
\ea
we have
\ba
...
W^{Xi}_{LM}(r)\,.
\ea
Thus, \cref{eq:cell_mixing_matrix_explicit} becomes
\ba
\mathcal{M}_{\ell nn'}^{XY,LNN'}
&=
\sum_{ij}
...
&=
\sum_{ij}
\mathcal{M}_{\ell nn'}^{XY,ij,LNN'}
\,,
\label{eq:Mij_same_gnl}
\ea
where $\mathcal{M}^{XY,ij}$ is the coupling matrix calculated from
the window terms $i$ and $j$. In this derivation, we assumed that the basis
functions $g_{n\ell}(r)$ are the same for all parts of the window.

What remains is to construct the decomposition \cref{eq:semi_separable_window}.
We do this recursively. First, we determine $\bar\phi(r)$ as an effective sky fraction. That is,
\ba
\bar \phi(r) &= f_\sky^\mathrm{eff}(z)
= \int\frac{\dd^2\hat r}{4\pi}\,W(\vr)\,.
\ea
We choose this because we also need to calculate the mixing matrix for
$W(\vr)/f_\sky^\mathrm{eff}(z)$.
Then, we construct the average angular window,
\ba
\bar M(\rhat) &= \frac{1}{\int\dd r\,w(r)} \int \dd r\, w(r)\, W(\vr)\,,
\ea
where $w(r)$ is a weighting scheme (e.g., $1$ or $r^2$).
Then we calculate the residual
\ba
\delta W(\vr) = W(\vr) - \bar\phi(r)\bar M(\rhat)\,.
\ea
We then perform the same procedure on this residual.
As a final step, we apply a normalization (since SuperFaB expects
$\max(W(\vr))=1$) to the angular terms.
